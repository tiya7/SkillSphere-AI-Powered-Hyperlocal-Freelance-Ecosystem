    ...
    <div style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
      ...
      <div style={{
        ...
        <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', opacity: 0.1 }} viewBox="0 0 500 700" preserveAspectRatio="xMidYMid slice">
          ...
          <line x1="120" y1="280" x2="80" y2="120" stroke={isClient ? '#16a34a' : '#6366f1'} strokeWidth="1.5"/>
            ...
            {[['client', '💼', 'I want to Hire'], ['freelancer', '🧑‍💻', 'I want to Work']].map(([r, icon, label]) => (
              <button key={r} onClick={() => setForm({ ...form, role: r })}
                style={{
                  ...
                  color: form.role === r ? '#0a2416' : 'rgba(255,255,255,0.7)',
                }}>
                {icon} {label}
              </button>
            ))}
        ...
        <div style={{ maxWidth: 400, width: '100%', margin: '0 auto' }}>
          ...
          <div style={{ marginBottom: 28 }}>
            ...
            <p style={{ color: '#71717a', fontSize: 14 }}>
              ...
              <Link to="/login" style={{ color: '#16a34a', fontWeight: 700 }}>Sign in</Link>
            </p>
          </div>

          {/* Role pills on form side */}
            ...
            {[['client', '💼', 'Hire Talent', 'Post gigs'], ['freelancer', '🧑‍💻', 'Find Work', 'Get hired']].map(([r, icon, label, sub]) => (
              <button key={r} type="button" onClick={() => setForm({ ...form, role: r })}
                style={{
                  ...
                  border: form.role === r
                    ? `2px solid ${r === 'client' ? '#16a34a' : '#06b6d4'}`
                    : '1.5px solid #e4e4e7',
                  background: form.role === r
                    ? r === 'client' ? '#f0faf4' : '#f0fdff'
                    : 'white',
                  transition: 'all 0.2s',
                }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <button type="submit" disabled={isLoading}
              style={{
                ...
                background: isClient
                  ? 'linear-gradient(135deg, #052e10, #16a34a)'
                  : 'linear-gradient(135deg, #0a1628, #06b6d4)',
                color: 'white', border: 'none', borderRadius: 12,
                fontSize: 15, fontWeight: 700,
                cursor: isLoading ? 'not-allowed' : 'pointer',
                opacity: isLoading ? 0.7 : 1,
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                boxShadow: isClient ? '0 4px 20px rgba(10,36,22,0.35)' : '0 4px 20px rgba(6,182,212,0.3)',
                transition: 'all 0.2s',
              }}>
              {isLoading ? 'Creating account...' : <>{isClient ? 'Create Client Account' : 'Create Freelancer Account'} <ArrowRight size={16} /></>}
            </button>
          </form>

          <p style={{ fontSize: 12, color: '#a1a1aa', textAlign: 'center', marginTop: 20, lineHeight: 1.6 }}>
            By registering you agree to our{' '}
            <Link to="/terms" style={{ color: '#16a34a' }}>Terms</Link> and{' '}
            <Link to="/privacy" style={{ color: '#16a34a' }}>Privacy Policy</Link>
          </p>
        </div>
      </div>
    </div>
